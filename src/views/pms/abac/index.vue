  ...
  <CommonPage>
    <template #action>
      <n-popover trigger="hover" class="ml-40">
        <template #trigger>
          <NButton type="info" size="small" @click="onClick">
            <n-icon size="20">
    ...
    >
      <template #resize-trigger>
        <div class="h-450 w-full flex items-center justify-center">
          <NButton v-if="splitSize !== 0" quaternary size="small" type="primary" @click="zoomOut()">
            <i class="i-material-symbols:switchRight mr-4 text-18" />
          </NButton>
          <NButton v-else size="small" quaternary type="primary" @click="zoomIn()">
      ...
      <template v-if="currentAbac" #2>
        <n-scrollbar style="max-height: 600px">
          <div class="w-full flex flex-col">
            <div class="mb-4 ml-20 mr-20 flex justify-between">
              <h3 class="mb-12">
                策略详情
              </h3>
              <n-space>
                <NSwitch v-model:value="securityExpression.enable" :loading="enableLoading" @update:value="handleSwitch">
                  <template #checked>
                    开启
                  </template>
                  <template #unchecked>
          ...
          <div
            class="ml-20 mr-20 mt-20 flex gap-4"
            :class="splitSize !== 0 ? 'flex-col' : 'flex-row'"
          >
            <n-collapse-transition :show="show" class="min-w-0 flex-1">
              <SpelContext
                v-model:env="env"
            ...
            <n-card title="策略规则" class="min-w-0 flex-1 overflow-hidden" hoverable>
              <template #header-extra>
                <n-space>
                  ...
                  <NButton size="small" type="error" @click="handleRunExpression">
...
import { findPolicyRule } from './apollo'
...
async function initAbac(isPreAuth = true) {
  ...
  if (currentAbac.value) {
    ...
    const { data } = await findPolicyRule(currentAbac.value.id, type.value === 'preAuth', 'network-only')
    securityExpression.value = {
