<template>
  <div v-if="triggeredByEvent" style="display: flex; align-items: center;">
    <NIcon size="30">
      <span class="bpmn-icon-event-subprocess-expanded" />
    </NIcon>
    <div style="display: flex; flex-direction: column; margin-left: 8px;">
      <span style="font-size: 12px">事件子流程</span>
      <span>{{ name }}</span>
    </div>
  </div>
  ...
  <div v-else style="display: flex; align-items: center;">
    ...
    <div style="display: flex; flex-direction: column; margin-left: 8px;">
      <span style="font-size: 12px">子流程</span>
      <span>{{ name }}</span>
    </div>
  </div>
  <NTabs type="bar" animated>
    ...
    <NTabPane v-if="!triggeredByEvent" name="inputs" tab="输入参数">
      <NScrollbar style="max-height: 600px">
        <Inputs :element="element" :modeler="bpmnModeler" />
      </NScrollbar>
    </NTabPane>
    <NTabPane v-if="!triggeredByEvent" name="Onputs" tab="输出参数">
      <NScrollbar style="max-height: 600px">
...
eventBus.on('commandStack.element.updateProperties.postExecute', (event) => {
  ...
  if (e.type === 'bpmn:SubProcess' && e.id === props.selectedElement.id) {
