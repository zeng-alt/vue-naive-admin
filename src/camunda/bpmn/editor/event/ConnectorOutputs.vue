        ...
        <NCard size="small" :bordered="true">
          ...
          <NSpace vertical>
            ...
            <NFormItem label="参数名" :show-feedback="false">
              <NInput v-model:value="output.name" size="small" :autofocus="index === outputData.outputs.length - 1" placeholder="输入参数名称" />
            </NFormItem>
            ...
            >
              <NInput
                ...
                :placeholder="output.type === 'expression' ? '${variable}' : '输入字符串值'"
              />
            </NFormItem>
            ...
            <template v-if="output.type === 'script'">
              <NFormItem label="脚本格式" :show-feedback="false">
                <NSelect v-model:value="output.scriptFormat" size="small" :options="scriptFormatOptions" />
              </NFormItem>
              <NFormItem label="脚本内容" :show-feedback="false">
                <NInput v-model:value="output.scriptValue" size="small" type="textarea" placeholder="输入脚本代码" :rows="4" />
              </NFormItem>
            </template>
...
function updateElement() {
  ...
  if (outputData.outputs.length > 0) {
    const outputParameters = outputData.outputs.map((output) => {
...
function saveOutputs() {
  // 验证参数名
  const invalidOutputs = outputData.outputs.filter(output => !output.name.trim())
