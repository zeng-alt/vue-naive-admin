        ...
        <NCard size="small" :bordered="true">
          ...
          <NSpace vertical>
            ...
            <NFormItem label="参数名" :show-feedback="false">
              <NInput v-model:value="input.name" size="small" :autofocus="index === inputData.inputs.length - 1" placeholder="输入参数名称" />
            </NFormItem>
            ...
            >
              <NInput
                ...
                :placeholder="input.type === 'expression' ? '${variable}' : '输入字符串值'"
              />
            </NFormItem>
            ...
            <template v-if="input.type === 'script'">
              <NFormItem label="脚本格式" :show-feedback="false">
                <NSelect v-model:value="input.scriptFormat" size="small" :options="scriptFormatOptions" />
              </NFormItem>
              <NFormItem label="脚本内容" :show-feedback="false">
                <NInput v-model:value="input.scriptValue" size="small" type="textarea" placeholder="输入脚本代码" :rows="4" />
              </NFormItem>
            </template>
              ...
              <NFormItem label="列表项" :show-feedback="false">
                <div class="list-items">
                  <div v-for="(item, itemIndex) in input.listItems" :key="itemIndex" class="list-item">
                    <NInputGroup>
                      <NInput v-model:value="item.value" size="small" :placeholder="getListItemPlaceholder(item.type)" />
                      <NButton
              ...
              <NFormItem label="键值对" :show-feedback="false">
                <div class="map-entries">
                  <div v-for="(entry, entryIndex) in input.mapEntries" :key="entryIndex" class="map-entry">
                    <NInputGroup>
                      <NInput v-model:value="entry.key" size="small" placeholder="键名" />
                      <NInput v-model:value="entry.value" size="small" :placeholder="getMapValuePlaceholder(entry.type)" />
...
// Emits
const emit = defineEmits(['inputsUpdated', 'save'])
...
function updateElement() {
  ...
  if (inputData.inputs.length > 0) {
    ...
    const inputParameters = inputData.inputs.map((input) => {
...
function saveInputs() {
  // 验证参数名
  const invalidInputs = inputData.inputs.filter(input => !input.name.trim())
  if (invalidInputs.length > 0) {
    window.$error('请填写所有参数的名称')
    return
  }
...
function loadInputData() {
  ...
  const extensionElements = props.bo.extensionElements

  if (extensionElements) {
    const inputOutput = extensionElements.values?.find(
      element => element.$type === 'camunda:Connector',
    ).inputOutput
